...
export default function AuthModal({ isOpen, onClose, onAuthSuccess, initialMode = 'login' }) {
  ...
  // Traditional Email/Password Auth & Forgot Password Reset
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-[#FFFFFF] dark:bg-[#151E2E] border border-[#E2E8F0] dark:border-[#243044] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 relative transition-all">
        ...
        {/* Divider (Shown only on Login & Signup) */}
        {mode !== 'forgot' && (
        ...
        {/* 2. TRADITIONAL EMAIL/PASSWORD & RESET FORM */}
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div>
            <label className="block text-[11px] font-bold text-[#0F172A] dark:text-[#F8FAFC] uppercase mb-1">
              {mode === 'forgot' ? 'New Password (نیا پاس ورڈ)' : 'Password'}
            </label>
            <div className="relative">
          ...
          >
            {loading 
              ? 'Authenticating...' 
              : (mode === 'forgot' 
                  ? 'Reset Password & Sign In (پاس ورڈ ری سیٹ کریں)' 
                  : mode === 'login' 
                    ? 'Sign In with Email' 
                    : 'Create Free Account')}
          </button>
